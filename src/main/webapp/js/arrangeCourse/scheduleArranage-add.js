/**
 * @author: LL
 * @createtime: 2018/5/11 12:00
 * @description:安排课程添加的js
 */
//初始化加载需要使用的layui模块
...
//查询任务课程信息
function getTaskArrangeCourseInfo(form){
    $.ajax({
        url : contextPath+'/arrangeCourse/getTaskArrangeCourse.action',
        data : {"arrangeCourseId":arrangeCourseId_all},
        type : 'POST',
        dataType : 'json',
        success : function (response){
            showTaskArrangeCourseInfo(response,form);
        }
    });
}
//显示任务课程信息
function showTaskArrangeCourseInfo(response,form){
    var courseAndYearTerm = response;
    var teacherClassInfoList = response.teachers;
    $("#v_courseName").text(courseAndYearTerm.courseName);
    $("#v_courseCode").text(courseAndYearTerm.courseCode);
    $("#v_academicYear").text(courseAndYearTerm.academicYear);
    $("#v_term").text(courseAndYearTerm.term);
    $("#v_campus").text(replaceStatus(courseAndYearTerm.campusCode));
    $("#v_totalPeriod").text(courseAndYearTerm.totalPeriod);
    $("#v_majorName").text(courseAndYearTerm.majorName);
    $("#v_majorStudentsNumber").text(courseAndYearTerm.majorStudentsNumber);
    var classNames = courseAndYearTerm.className;
    var classNameArr = classNames.split(",");
    $("#classNames").empty();//清空表格中数据并重新填充数据
    for(var i=0,length_l = classNameArr.length; i<length_l; i++){
        var inputCheck = "<input type='checkbox' name='className' value='"+classNameArr[i]+"' lay-skin='primary' title='"+classNameArr[i]+"'>";
