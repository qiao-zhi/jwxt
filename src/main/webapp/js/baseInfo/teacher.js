...
function findTeacherBaseInfo(){
    $.ajax({
        ...
        data : $("#selectTeacherInfoForm").serialize(),
        type : 'POST',
        dataType : 'json',
        async:true,
        success : showTeacherBaseInfo
    });
}
function showTeacherBaseInfo(pageInfo){
        ...
        laypage.render({
            ...
            jump: function(obj, first){//点击页号的时候执行的函数
                ...
                if(!first){//首次不执行(点击的时候才执行)
                    findTeacherBaseInfo();//执行查询分页函数(这个函数必须写在这里)
                }
            }
        });
    });
...
function deleteTeacherInfo(id) {
    layer.confirm('确认要删除吗？', function (index) {
        $.ajax({
            url:contextPath+"/teacherInfo/deleteTeacherInfo.action",
            type:"post",
            dataType:"text",
            data:{"teacherId":id},
...
//新增任务
function addTeacher(){
    notice_tab_show('新增教师','./teacher-add.jsp')
}
